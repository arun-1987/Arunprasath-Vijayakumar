...
public class WaitHelper {
	 ...
	 * @return
	 */
	public boolean waitForIsDisplayed(WebDriver driver,WebElement element, int timeout, int polling) {
		logger.info("Waiting for " + element.toString() + " for the specified:" + timeout + " seconds");
		WebDriverWait wait = getWait(driver,timeout, polling);
	...
	public void isDisplayedUsingAwait(WebDriver driver, By locator) {
		await().atMost(TEN_SECONDS).pollInterval(1, TimeUnit.SECONDS)
	...
	public void isNotDisplayedUsingAwait(WebDriver driver, By locator) {
		await().atMost(TEN_SECONDS).pollInterval(1, TimeUnit.SECONDS)
				.ignoreException(StaleElementReferenceException.class)
				.untilCall(isDisplayed(driver,locator), is(false));
	}
	
	private Callable<Boolean> pageTitleMatches(WebDriver driver, final String searchString) {
